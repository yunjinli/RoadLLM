...
def get_coco_annotation_from_obj(obj, label2id):
    ...
    if label not in label2id:
        print(f"[Warning] {label} is not in {label2id} !")
        return None
...
def convert_xmls_to_cocojson(annotation_paths: List[str],
    ...
    assert len(annotation_paths) == annotation_id.shape[0]
    output_json_dict = {
        "images": [],
        "type": "instances",
        "annotations": [],
    ...
    for i, a_path in tqdm(enumerate(annotation_paths)):
        ...
        img_info['id'] = int(annotation_id[i])
        img_id = img_info['id']
        # img_id = int(annotation_id[i])
...
def main():
    ...
    parser.add_argument('--annotation_ids', type=str, default=None,
                        )
    ...
    annotation_id = np.load(args.annotation_ids)
