...
def preprocess_qwen3(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    # roles = {"human": "<|im_start|>user", "gpt": "<|im_start|>assistant"}
    roles = {"human": "user", "gpt": "assistant"}
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if any(
        ...
        cfg_pretrained = AutoConfig.from_pretrained(model_args.model_name_or_path) ## Get the LLM config
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        rank0_print(f"Total parameters: ~{total_params/1e6:.2f} MB / {total_params} parameters")
        rank0_print(f"Trainable parameters: ~{trainable_params/1e6:.2f} MB / {trainable_params} parameters")
        rank0_print(f"{(trainable_params / total_params * 100):.2f} % parameters are trained")
        if training_args.bits in [4, 8]:
